...
class NotificationSettingController extends Controller
{
                                  ...
                                  NotificationModulesModel $modules)
    {
		...
		$this->module_url_path 	          = url(config('app.project.role_slug.student_role_slug')."/notification_settings");
		$this->module_view_folder         = "student.notification_settings";
        ...
        $obj_user = $this->StudentModel->where('user_id',$this->user_id)->where('school_id',$this->school_id)->where('academic_year_id',$this->academic_year)->where('is_active',1)->where('has_left',0)->first();
        if($obj_user)
        {
            $this->id = $obj_user->id;
        }
    ...
    */
    public function index(){
        ...
        $obj_modules = $this->NotificationModulesModel
                            ->where('role',config('app.project.role_slug.student_role_slug'))
                            ->where('is_active',1)
                            ->get();
