...
class UserController extends Controller
{
    ...
    {     
        ...
        if(isset($arr_search_column['q_contact_number']) && $arr_search_column['q_contact_number']!="")
        {
            ...
            $obj_user = $obj_user->where($user_details.'.contact_number','LIKE', '%'.$search_term.'%');
        }

        if(isset($arr_search_column['q_last_login']) && $arr_search_column['q_last_login']!="")
        {
            $search_term      = $arr_search_column['q_last_login'];
            $obj_user = $obj_user->where($user_details.'.last_login','LIKE', '%'.$search_term.'%');
        }
        ...
        $json_result     = $json_result->editColumn('last_login',function($data)
                            ...
                            ->editColumn('build_action_btn',function($data) use ($role)
                            {
                                ...
                                {       
                                    ...
                                    {   
                                        $build_status_btn = '<a class="btn btn-circle btn-dangers  btn-bordered btn-fill show-tooltip " title="Lock" href="'.$this->module_url_path.'/activate/'.base64_encode($data->id).'" 
