...
class NotificationSettingController extends Controller
{
                                  ...
                                  NotificationModulesModel $modules)
    {
		...
		$this->module_url_path 	          = url(config('app.project.role_slug.school_admin_role_slug')."/notification_settings");
		$this->module_view_folder         = "schooladmin.notification_settings";
        ...
        $this->role                       = \Session::has('role')?\Session::get('role'):'';
        ...
        $obj_user = $this->SchoolAdminModel->where('user_id',$this->user_id)->where('school_id',$this->school_id)->where('is_active',1)->first();
        if($obj_user)
        {
            $this->id = $obj_user->id;
        }
        ...
        $page_title =  str_plural($this->module_title);
          ...
          /*$obj_modules = $this->ModuleUserModel
                            ->whereHas('get_role',function($q){
                                $q->where('slug',config('app.project.role_slug.school_admin_role_slug')) ;   
                            })
                            ->whereHas('get_modules',function($q){
                            		$q->where('is_show_in_notification','=','1');
                            })
                            ->with(['get_role','get_modules'])
							->get();*/

        $obj_modules = $this->NotificationModulesModel
                            ->where('role',config('app.project.role_slug.school_admin_role_slug'))
                            ->where('is_active',1)
                            ->get();
