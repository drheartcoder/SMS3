...
class TransactionController extends Controller
{
	public function __construct(CommonDataService $CommonDataService,
        ...
        $this->arr_view_data     = [];
        $this->module_url_path   = url(config('app.project.school_admin_panel_slug')).'/transactions';
        ...
        $this->module_view_folder           = "schooladmin.transactions";
    ...
    */ 
    ...
    {
        $obj_custom = $this->get_transaction_records($request,$type);
        ...
        $json_result =  $json_result
                        ...
                        ->editColumn('build_approval',function($data) use ($arr_current_user_access)
                        {
                        	$str ='';
    ...
    public function get_transaction_records(Request $request,$fun_type='')
    {
        ...
        if($request->has('search') && $search_term!="")
        {
            $obj_custom = $obj_custom
                                     ->whereRaw("( ( CONCAT(".$user_translation.".first_name,' ',".$user_translation.".last_name) LIKE '%".$search_term."%') ")
    ...
    */
    ...
    public function change_status(Request $request){
    	...
    	if($request->has('id') && $request->has('status')){
            ...
            if($obj_transaction->type=='CANTEEN'){
                $canteen_status = $status;
                if($status=='APPROVED')
                    $canteen_status = 'DONE';
                $this->CanteenBookingsModel->where('order_no',$obj_transaction->order_no)->update(['payment_status'=>$canteen_status]);
            }

            if($request->has('reason')){
                $obj_transaction->rejection_reason  = $request->input('reason');
            }
            ...
            \Excel::create(ucwords($this->module_title).'-'.date('d-m-Y').'-'.uniqid(), function($excel) use($obj_data) 
                {
                    $excel->sheet(ucwords($this->module_title), function($sheet) use($obj_data) 
                    {
                        ...
                        $arr_fields['user_no']         = translation('user_no');
                        ...
                        if(sizeof($obj_data)>0) 
                        {
                            ...
                            foreach($obj_data as $key => $result)
                            {
                                ...
                                $arr_tmp[$key]['payment_done_by'] = ucwords($result->parent_name);
                                $arr_tmp[$key]['user_no']         = $result->user_no;
                                $arr_tmp[$key]['payment_date']    = $result->payment_date;
                                $arr_tmp[$key]['payment_mode']    = $result->transaction_type;
                                $arr_tmp[$key]['status']          = translation(strtolower($result->approval_status));
