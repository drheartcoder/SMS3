...
class CanteenBookingsController extends Controller
{
    ...
    {   
        ...
        $this->module_url_path              = url(config('app.project.school_admin_panel_slug')."/canteen_bookings");
        

        $this->module_title                 = translation("canteen_bookings");
        $this->module_view_folder           = "schooladmin.canteen_bookings";
    ...
    function get_pre_bookings(Request $request)
    {
        $school_id = $this->school_id;
    ...
    public function print_doc($enc_id)
    {
        ...
        $this->arr_view_data['module_icon']          = $this->module_icon;
        $this->arr_view_data['view_icon']            = $this->view_icon;
        $this->arr_view_data['school_admin_panel_slug'] = $this->school_admin_panel_slug;
    ...
    public function status($enc_id)
    {
        ...
        if($obj_records)
        {
            ...
            if($records)
            {
                Flash::success($this->module_title.' '.translation('deleverd_successfully'));
            }
            else
            {
                Flash::success($this->module_title.' '.translation('deleverd_successfully'));
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if(isset($cart_data) && $cart_data != null)
        {
             ...
             $daily_meal = $this->DailyMealsModel
                                ->with('weekly_meal.get_product_details')
                                ->whereHas('weekly_meal.get_product_details',function($q) use($product_id){
            ...
            $available_stock =   isset($daily_meal->available_stock)?$daily_meal->available_stock:0;
        ...
        if(isset($data) && $data != null)
        {
            ...
            {   
                ...
                if($quantity <= $available)
                {
                    ...
                    return response()->json(array('status'=>'success','data'=>$quantity,'price'=>$total,'total_price'=>$total_amount));
                }
                else
                {
                    return response()->json(array('status'=>'error','data'=>$data->quantity,'price'=>$data->price,'total_price'=>$total_amt));
                }
            }
            if($status == 'decrement')
            {
    ...
    */
    ...
    {        
        ...
        $this->arr_view_data['module_icon']     = $this->module_icon;
        $this->arr_view_data['view_icon']       = $this->view_icon;
        $this->arr_view_data['user_role']       = $request->input('user_type');
        $this->arr_view_data['user_id']         = $request->input('users');
    ...
    */
    public function store(Request $request)
    {
            ...
            $arr_data['customer_id']            =   $user_id;
            $arr_data['staff_id']               =   $this->user_id;
            $arr_data['user_role']              =   $request->input('user_type');
        ...
        if($create_booking)
        {
            ...
            if($selector == 'cash')
            {
                ...
                $arr_translation['staff_id']        = $this->user_id;
                $arr_translation['amount']          = $request->input('total');
                ...
                if($transaction)
                {
                        ...
                        $arr_transaction_details ['amount']           = $request->input('total');
    ...
    */
    ...
    /*
    | change_payment_status() : change payment status at Canteen_booking table and generate transaction of cash in tbl_canteen_transaction table
    | Auther  : sayali B
    | Date    : 22-06-2018
    */
    public function change_payment_status(Request $request)
    {
        ...
        $this->TransactionDetailsModel->where('order_no',$order_no)->update(['approval_status'=>'APPROVED']);
    }
