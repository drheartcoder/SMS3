...
class CanteenBookingsController extends Controller
{
    ...
    {   
        ...
        $this->module_url_path              = url(config('app.project.parent_panel_slug')."/canteen_bookings");
        

        $this->module_title                 = translation("canteen_bookings");
        $this->module_view_folder           = "parent.canteen_bookings";
        ...
        $this->module_icon                  = "fa-cart-arrow-down";
    ...
    function get_pre_bookings(Request $request,$fun_type='')
    {
        ...
        /*dd($obj_pre_bookings->get());*/
        /* ---------------- Filtering Logic ----------------------------------*/                    
        
        $arr_search_column = $request->input('column_filter');

        if(isset($arr_search_column['q_user']) && sizeof($arr_search_column['q_user'])>0 && $arr_search_column['q_user']!="")
        {
            $search_term      = $arr_search_column['q_user'];
            ...
            $obj_pre_bookings = DB::table($booking_table)
                            ...
                            ->where($booking_table.'.academic_year_id','=', $this->academic_year)
                            ->where($user_table.'.id','=', $this->user_id)
                            ->where($booking_table.'.user_role','=', $search_term)
                            ->whereNull($booking_table.'.deleted_at')
                            ->leftJoin($user_table,$user_table.'.id',' = ',$booking_table.'.customer_id')
                            ->leftJoin($user_trans_table,$user_trans_table.'.user_id', ' = ',$user_table.'.id')
                            ->groupBy('id')
    ...
    public function print_doc($enc_id)
    {
        ...
        $this->arr_view_data['module_icon']          = $this->module_icon;
        $this->arr_view_data['view_icon']            = $this->view_icon;
        $this->arr_view_data['parent_panel_slug'] = $this->parent_panel_slug;
    ...
    */
    public function checkout(Request $request)
    {
        ...
        $this->arr_view_data['module_icon']     = $this->module_icon;
        $this->arr_view_data['view_icon']       = $this->view_icon;
        $this->arr_view_data['date']            = $request->input('start_date');
                ...
                foreach ($arr_details as $key => $details) 
                {
                    ...
                    if(isset($data1) && count($data1)>0)
                    {
                        ...
                        if($arr_data['available_stock'] > 0)
                        {
                            ...
                            if($available<0)
                            {
                                Flash::error(translation('stock_is_available_for').' '.$arr_data['weekly_meal']['get_product_details']['product_name'].' '.translation('please_remove_product_from_cart'));
    ...
    */
    public function get_users(Request $request)
    {
     ...
     /*
    | get_cart_data() : get cart items related to selected use if present
    | Auther  : sayali B
    | Date    : 22-06-2018
    */
    ...
    */
    ...
    /*
    | change_delivery_status() : change delivery status at Canteen_booking table 
    | Auther  : sayali B
    | Date    : 22-06-2018
    */
    public function change_delivery_status(Request $request)
    {
        ...
        $this->CanteenBookingsModel->where('id',$id)->update(['delivery_status'=>$status]);    
       
    }

    /*
    | change_payment_status() : change payment status at Canteen_booking table 
    | Auther  : sayali B
    | Date    : 22-06-2018
    */
    public function change_payment_status(Request $request)
    {
        ...
        $this->CanteenBookingsModel->where('id',$id)->update(['payment_status'=>$status]);    
       
    }

    /*
    | get_meals() : get daily meals as per selected date
    | Auther  : sayali B
    | Date    : 25-06-2018
    */
    public function get_meals(Request $request)
    {
        ...
        if(isset($daily_meals_data) && $daily_meals_data != null)
        {
            $arr_daily_meal  =  $daily_meals_data->toArray();
        }  

        $data .= '<div class="row">';
        $data .= '<div class="details-infor-section-block">'.translation('canteen_products').'</div>';
        ...
        if(isset($arr_daily_meal) && count($arr_daily_meal)>0)
        {
            ...
            {
                $data .= '<div class="col-sm-6 col-md-4 col-lg-4 cart_item ">';
                $data .= '<div class="Latest-cou-list-blo">';
                $data .= '<div class="Latest-cou-img-blo">';
                    ...
                    { 
                        ...
                        $data .='<img src="'.$product_image.'" class="Latest-list-img" alt=""/>';
                    }
                    else
                    {
                        $product_image = '';
                    ...
                    $data .= '<div class="view-info-over">';
                    $data .= '<a  href="javascript:void(0);"';
                            $data .='data-daily-id="';
                                    $data .= isset($daily_meal['id'])? $daily_meal['id']:'';
                            $data .='"';
                            $data .=' data-id="' ;
                                    $data .= isset($daily_meal['weekly_meal']['item_id'])? $daily_meal['weekly_meal']['item_id']:'';
                            $data .='"';
                            $data .=' data-price="';
                                    $data .= isset($daily_meal['weekly_meal']['get_product_details']['price'])?$daily_meal['weekly_meal']['get_product_details']['price']:'';
                            $data .='" data-qty="1"';
                            $data .='data-name="';
                                    $data.= isset($daily_meal['weekly_meal']['get_product_details']['product_name'])? $daily_meal['weekly_meal']['get_product_details']['product_name']:'';
                            $data .= '" data-maxqty="';
                                    $data.=isset($daily_meal['stock'])? $daily_meal['stock']:'';
                            $data .= '" data-sold-stock="';
                            $data .= isset($daily_meal['stock_sold'])? $daily_meal['stock_sold']:'';
                            $data .= '" onclick="add_to_cart(this)"><i class="fa fa-cart"></i>'.translation('add_to_cart').'</a></div>';
    
                    $available = $daily_meal['available_stock'];
                                          
                    $data .= '<input type="hidden" name="available" id="available_'.$daily_meal['id'].'" value="'.$available.'">';
                    $data .= '</div>';
                    $data .= '<div class="Latest-cou-text-blo margin-remve ">';
                    $data .= '<a class="title-products">';
                            $data .= isset($daily_meal['weekly_meal']['get_product_details']['product_name'])?$daily_meal['weekly_meal']['get_product_details']['product_name']:'-';
                    $data .='</a>';
                                          
                    $data .= '<div class="address-block-new">';
                            $data .= isset($daily_meal['weekly_meal']['get_product_details']['description'])?$daily_meal['weekly_meal']['get_product_details']['description']:'-';
                    $data .= '</div>';
                                          
                    $data .= '<div class="address-block-new"><i class="fa fa-clock-o"></i> ';
                            $data .= isset($daily_meal['weekly_meal']['get_product_details']['price'])?$daily_meal['weekly_meal']['get_product_details']['price']:'-';
                            $data .=' MAD ';
                    $data .= '</div></div><div class="clearfix"></div></div></div>';
                }
            }
            else
            {                
                    $data .= '<div class="col-sm-6 col-md-8 col-lg-8">';
                    $data .= '<div class="Latest-cou-list-blo">';
                    $data .= '<div><h4 align="center" style="color: red">'.translation('oops').'! '.translation('no_products_available_for_today').'</h4></div></div></div>';
            }
            $data .='</div>';
