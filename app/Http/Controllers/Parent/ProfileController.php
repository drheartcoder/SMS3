...
class ProfileController extends Controller
{
   	...
   	public function index()
    {
    	$arr_data = '';
        $id       = $this->user_id;
        $obj_data = $this->UserModel->with(['translations','get_parent_details'=>function($query)use($id){
    ...
    public function edit_profile()
    {
        $arr_data = array();
        $id       = $this->user_id;
        $obj_data = $this->UserModel->with(['get_parent_details'=>function($query)use($id){
                                             $query->where('user_id','=',$id);
                                     }])
                                    ->where('id',$id)
                                    ->first();       

        $parent   = $this->ParentModel->where('user_id',$id)->first();
        if($obj_data)
        {
            $arr_data = $obj_data->toArray();          
        } 
        ...
        $this->arr_view_data['arr_data']                      = $arr_data;
        $this->arr_view_data['parent']                        = $arr_parent;
    ...
    public function update(Request $request)
    {
        ...
        $arr_rules['alternate_mobile_no']           = "required|numeric|digits_between:10,14";
       ...
       $messages = array(
                            ...
                            'alternate_mobile_no.digits_between' => translation('please_enter_mobile_no_within_range_of_10_14'),
        ...
        $parent_data['occupation']          = trim($request->input('occupation'));
        $parent_data['qualification_degree']= trim($request->input('qualification_degree'));
        $parent_data['relation']            =  trim($request->input('relation'));
        $parent_data['marital_status']      =  trim($request->input('marital_status'));
        $parent_data['alternate_mobile_no'] =  trim($request->input('alternate_mobile_no'));

        $this->ParentModel->where('user_id',$user_id)->update($parent_data);
        ...
        {   
            ...
            $db_language        = $this->ParentModel->where('user_id',$user_id)->first();
            if($language)
            {
                if($language != $db_language->language)
                {
                    $this->ParentModel->where('user_id',$user_id)->update(['language'=>$language]);    
                    \Session::flush();
                    \Sentinel::logout();
                    return redirect(url($this->parent_url_path));
                }
            }
             Flash::success('Profile Updated successfully.');
        }
    ...
    */
     public function built_mail_data($arr_data)
    {
        ...
        {
            $link = \URL::to(config('app.project.parent_panel_slug').'/email_change/'.base64_encode($arr_data['id']));
    ...
    public function set_language(Request $request)
    {
        ...
        $db_language        = $this->SchoolParentModel->where('parent_id',$this->user_id)->first();
        
        if($language)
        {
            if($language != $db_language->language)
            {
                $this->SchoolParentModel->where('parent_id',$this->user_id)->update(['language'=>$language]);
