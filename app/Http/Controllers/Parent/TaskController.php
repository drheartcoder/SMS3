...
class TaskController extends Controller
{
     public function __construct(CommonDataService $CommonDataService,EmailService $EmailService){
    	...
    	$this->arr_view_data                = [];
        $this->module_url_path              = url(config('app.project.parent_panel_slug')).'/task';
        $this->module_title                 = translation('task');
        
        $this->module_view_folder           = "parent.task";
    ...
    public function index(){
        ...
        $obj_tasks = $this->TaskModel
                                    ...
                                    ->with('get_user')
                                    ->with('get_supervisor')
                                    ->whereRaw('user_role="'.config('app.project.role_slug.parent_role_slug').'" and school_id="'.$this->school_id.'" and academic_year_id="'.$this->academic_year.'" and ( level_class_id="0" or level_id="0" or level_class_id="'.$this->level_class_id.'" or level_id='.$level.' )')
                                    ->orderBy('id','DESC')
                                    ->get();                            
    ...
    public function change_user_status(Request $request){
        ...
        $task_detail = $this->TaskModel->where('id',$task->task_id)->first();
