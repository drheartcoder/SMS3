...
class KeywordTranslationController extends Controller
{
    ...
    function get_keyword_records(Request $request)
    {
        ...
        if($request->has('search') && $search_term!="")
        {
            ...
            $obj_keyword = $obj_keyword->WhereRaw("( (".$prefixed_keyword_table .".keyword LIKE '%".$search_term."%') ")
                                     ->orWhereRaw("(".$prefixed_keyword_table.".title LIKE '%".$search_term."%')  )");
        }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($arr_lang as $key => $value) 
        {
            ...
            if($value['locale'] == 'fr')
            {
                $arr_insert['fran&ccedil;ais']    = $request->input('franccedilais');
            }
