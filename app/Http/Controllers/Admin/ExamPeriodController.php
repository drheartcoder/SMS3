...
class ExamPeriodController extends Controller
{
    							...
    							)
    {
		...
		$this->BaseModel                  = $this->ExamPeriodModel;
		$this->ExamPeriodTranslationModel = $exam_period_translation;
		$this->module_url_path 	          = url(config('app.project.admin_panel_slug')."/exam_period");
		$this->module_view_folder         = "admin.exam_period";
    ...
    public function get_records(Request $request)
    {
        ...
        $json_result =  $json_result->editColumn('enc_id',function($data) use ($current_context,$arr_current_user_access)
                        ...
                        ->editColumn('exam_name',function($data) use ($current_context,$arr_current_user_access)
                            { 
                                 
                                if($data->exam_name!=null && $data->exam_name!=''){
                                    return  ucfirst($data->exam_name);
                                }else{
                                    return  '-';
                                }
    ...
    public function get_exam_period_records(Request $request)
    {
        ...
        $obj_custom = DB::table($custom_table)
                        ->select(DB::raw(   
                                        ...
                                        ->leftJoin($custom_translation_table,$custom_translation_table.'.exam_id',' = ',$custom_table.'.id');
        ...
        if($status)
        {
            ...
            if(sizeof($arr_lang) > 0 )
            {
                ...
                {            
                    ...
                    $slug = strslug($exam_name);

                    if( (isset($exam_name)  && $exam_name != ''))
                    { 
                        $translation = $status->translateOrNew($lang['locale']);

                        $translation->exam_id   = $status->id;
                        $translation->exam_name  = $exam_name;
            ...
            if(sizeof($arr_lang) > 0 )
            {
                ...
                {            
                    ...
                    { 
                        $translation = $status->translateOrNew($lang['locale']);
                        $translation->exam_id    = $status->id;
                        $translation->exam_name  = $fee_name;
                        $translation->slug       = strslug($fee_name); 
                        $translation->save();
