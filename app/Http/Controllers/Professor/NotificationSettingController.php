...
class NotificationSettingController extends Controller
{
                                  ...
                                  NotificationModulesModel $modules)
    {
		...
		$this->module_url_path 	          = url(config('app.project.role_slug.professor_role_slug')."/notification_settings");
		$this->module_view_folder         = "professor.notification_settings";
        ...
        if($obj_data){
         ...
         $obj_user = $this->ProfessorModel->where('user_id',$this->user_id)->where('school_id',$this->school_id)->where('is_active',1)->where('has_left',0)->first();
         if($obj_user)
         {
            $this->id = $obj_user->id;
         }
    ...
    */
    public function index(){
        ...
        $obj_modules = $this->NotificationModulesModel
                            ->where('role',config('app.project.role_slug.professor_role_slug'))
                            ->where('is_active',1)
                            ->get();
