...
<div id="breadcrumbs">
  <ul class="breadcrumb">
     ...
     <li>
            ...
            <a href="{{url('/')}}/{{ config('app.project.professor_panel_slug') }}/dashboard">{{translation('dashboard')}}</a>
        </li>
        <span class="divider">
        ...
        <li> 
          ...
          <a href="{{$module_url_path}}">{{ isset($page_title)?$page_title:"" }}</a>
        </li>
...
<div class="page-title new-agetitle">
    <div>
        <h1> <i class="{{$view_icon}}"></i> {{$page_title}}</h1>
    </div>
</div>
<!-- END Page Title -->
...
<div class="col-md-12">
   <div class="box box-navy_blue">
        <div class="box-title">
            <h3><i class="fa fa-reply"></i>{{$module_title}}</h3>
            <div class="box-tool">
            </div>
        </div> 
         <div class="box-content">            
                ...
                <div class="form-group img-view-pro">
                       ...
                       @if(!empty($arr_data['get_survey_images']))
                        <div class="img-pro-vw">
                           ...
                           <div id="myCarousel" class="carousel slide" data-ride="carousel">
                              ...
                              <a class="left carousel-control" href="#myCarousel" data-slide="prev">
                                ...
                                <span class="sr-only">{{translation('previous')}}</span>
                              </a>
                              <a class="right carousel-control" href="#myCarousel" data-slide="next">
                                <span class="glyphicon glyphicon-chevron-right"></span>
                                <span class="sr-only">{{translation('next')}}</span>
                              </a>
                            </div>
                        ...
                        <div class="pr-viw-contents">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-12 col-lg-12">
                                    <div class="sb-titl-pro-surv">
                                        <div class="left-fessor-vw-left">{{ translation('end_date') }}</div>
                                ...
                                <div class="col-sm-12 col-md-12 col-lg-12">
                                    <div class="sb-titl-pro-surv">
                                        <div class="left-fessor-vw-left" style="font-size:10px; font-weight:400 !important ;float:right">{{translation('note')}} : {{translation('once_you_give_reply_you_cant_edit_it')}}</div>
                    ...
                    <div class="col-sm-9 col-md-8 col-lg-7 col-lg-offset-3 controls">
                       <form method="POST" action="{{ $module_url_path }}/store_survey_reply/{{$enc_id}}" accept-charset="UTF-8" class="form-horizontal"  enctype="multipart/form-data"  id="validation-form1" onsubmit="return addLoader()">
                        {{ csrf_field() }}
                        <div class="row">
                                ...
                                <div class="xpanel xpanel-info">
                                    ...
                                    <div id="collapse{{$arr_dataRs['id']}}" class="panel-collapse collapse @if($key==0) in @endif" aria-expanded="true" style="">
                                        <div class="panel-body">
                                             <?php
                                              $optionType   = '';
                                              $questionType = $arr_dataRs['get_question_type']['id'];
                                              if($questionType == 1){ ?>

                                                 <div class="form-group">
                                                      <div class="col-sm-12 col-lg-12 controls">
                                                          <input class="form-control" name="answer[{{$arr_dataRs['id']}}]" id="answer{{$arr_dataRs['id']}}" placeholder="{{translation('enter')}} {{translation('survey')}} {{ translation('reply')}}"  type="text" 
                                                          data-rule-required='true'>
                                                      </div>
                                                       
                                                 </div>     
                                              <?php }else if($questionType == 2){ ?>
                                                  <div class="form-group">
                                                      <div class="col-sm-12 col-lg-12 controls">
                                                          <textarea class="form-control" name="answer[{{$arr_dataRs['id']}}]" id="answer[{{$arr_dataRs['id']}}]"  placeholder="{{translation('enter')}} {{translation('survey')}} {{ translation('reply')}}" data-rule-required='true'></textarea>
                                                      </div>
                                                 </div>     
                                              <?php } else if($questionType == 3){
                                                    $optionType = 'radio';
                                                    $optionName = 'answer['.$arr_dataRs['id'].']';
                                                ...
                                                if(!empty($arr_options) && $optionType!=''){
                                                    foreach($arr_options as $key2 => $value ){ ?>
                                                    <div class="col-sm-12 col-md-12 col-lg-12">
                                                        <div class="sb-titl-pro-surv xxques-options-section">
                                                           
                                                            @if($questionType == 3)
                                                            <div class="radio-btn replay-positions">
                                                                <input id="rad{{$key2}}" name="{{$optionName}}"  data-rule-required='true' value="{{ $value }}"   type="radio"   data-rule-required='true'>
                                                                <label for="rad{{$key2}}">{{ $value or '' }}</label>
                                                                <div class="check"></div>
                                                            </div>
                                                            @endif
                                                             ...
                                                             @if($questionType == 4)
                                                             <div class="check-box position-div">

                                                               <input class="filled-in case chk" id="{{$key2}}" name="{{$optionName}}"  data-rule-required='true' value="{{ $value }}"   type="checkbox" data-rule-required='true'>

                                                              
                                                              <label for="{{$key2}}">{{ $value or '' }}</label>
                                                            </div>
                                                            @endif
