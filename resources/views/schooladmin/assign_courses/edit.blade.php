                           ...
                           @include('schooladmin.layout._operation_status')
                            <form method="post" onsubmit="return addLoader()"  action="{{$module_url_path}}/update/{{$enc_id}}"  class="form-horizontal" id="validation-form1">
                              ...
                              <?php
                                  ...
                                  $selectedLevle = isset($arr_data['level_id'])?$arr_data['level_id']:'';
                                  $selectedCls = isset($arr_data['class_id'])?$arr_data['class_id']:'';
                                  
                                ?>
                                <div class="form-group">
                                    <label class="col-sm-3 col-lg-2 control-label">{{translation('assign_level')}}<i class="red">*</i></label>
                                    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-4 controls">
                                          {{-- <div class="frmSearch">
                                            ...
                                            @if(!empty($arr_level) && count($arr_level) > 0)
                                                ...
                                                <option value="{{ $val['level_id'] }}" @if($val['level_id']==$selectedLevle) selected="selected" @endif  >{{ ucwords($val['get_level']['level_name']) }}
                                                </option>    
                                                @endforeach
                                            @endif
                                             </select>
                                          ...
                                          <input type="hidden" name="assign_level" value="{{isset($arr_data['level_id'])?$arr_data['level_id']:0}}">
                                          <input type="text" name="level" id="level" value="{{isset($arr_data['get_level']['level_name'])?$arr_data['get_level']['level_name']:''}}" readonly style="cursor: pointer;" class="form-control">
                                        <span class="help-block"></span>
                                    </div>
                                </div>
                               
                                <div class="form-group">
                                    <label class="col-sm-3 col-lg-2 control-label">{{translation('assign_class')}}<i class="red">*</i></label>
                                    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-4 controls">
                                          {{-- <div class="frmSearch">
                                          ...
                                          <input type="hidden" name="assign_class" value="{{isset($arr_data['class_id'])?$arr_data['class_id']:0}}">
                                          <input type="text" name="class" id="class" value="{{isset($arr_data['get_class']['class_name'])?$arr_data['get_class']['class_name']:''}}" readonly style="cursor: pointer;" class="form-control">
                                        <span class="help-block"></span>
                                    </div>
                                </div>
                                  ...
                                  <div class="col-lg-4">      
                                     ...
                                     <select class="js-example-basic-multiple form-control" multiple="multiple" name="subject[]" data-rule-required='true' id="subjects">
                                       @foreach($arr_courses as $key => $value)
                                          <option value="{{$value['get_course']['id']}}" @if(in_array($value['get_course']['id'],$subject) ) selected="selected" @endif>{{$value['get_course']['course_name'] or ''}}</option>
                                       @endforeach
                                     </select>
                                     <span class="help-block"> </span>
                                  </div> 
