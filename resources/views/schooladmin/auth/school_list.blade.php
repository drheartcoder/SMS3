                ...
                <div class="login-wrapper">
                    <div class="main-loginpage">
                        ...
                        <div class="login-content">
                           ...
                           <form id="validation-form1" onsubmit="return addLoader()" action="{{ url($school_admin_panel_slug.'/login_process') }}" method="post" >
                                ...
                                @endif
                                {{ csrf_field() }}
                                <h3>{{translation('select_school',$locale)}}</h3>
                                <hr>

                                <div class="" id="message"></div>
                                 <div class="form-group">
                                    <label for="school_id">{{translation('school_name',$locale)}}</label>
                                    <select name="school_id" class="form-control" data-rule-required="true">
                                        
                                            <option value="">{{translation('select_school',$locale)}}</option>
                                            @if(isset($arr_school)&&count($arr_school)>0)
                                ...
                                <div class="form-group">
                                    <div class="controls btn-loginpg">
                                        <button type="submit" class="btn btn-primary" id="submit_button">{{translation('go',$locale)}}</button>
                                    </div>
                                </div>
