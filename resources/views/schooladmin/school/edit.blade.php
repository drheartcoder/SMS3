          ...
          <div class="box-title">
           ...
           <div class="box-content edit-btns">
              ...
              <div class="tabbable">
                  ...
                  <form class="form-horizontal" onsubmit="return addLoader()"  name="validation-form1" id="validation-form1" method="POST" action="{{$module_url_path}}/update/{{$enc_id}}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                                ...
                                @foreach($arr_template as $key => $template)
                                  ...
                                  @if(isset($template['is_active']) && $template['is_active']==1)
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-3 col-md-4 col-lg-3 control-label" for="state">{{$label}} 
                                            ...
                                            <div class="col-sm-5 col-lg-4 controls">
                                              @if($template['get_question_category']['slug'] == 'multiple')
                                                  ...
                                                  @foreach($arr_options as $option)
                                                      <label><input type="checkbox" name="{{strslug($label)}}[]" id="{{strslug($template['title'])}}_{{$count++}}" value="{{$option}}" @if($value == $option) checked @endif
                                                        />
                                                      {{$option}} &nbsp; &nbsp;</label>
                                              ...
                                              @elseif($template['get_question_category']['slug'] == 'single') 
                                                  ...
                                                  ?>
                                                  @foreach($arr_options as $option) 
                                                     <label> <input type="radio" name="{{strslug($label)}}" value="{{$option}}" @if($value == $option) checked @endif > {{$option}} &nbsp;&nbsp;</label>
                                                  @endforeach  

                                              @elseif($template['get_question_category']['slug'] == 'short')
                                                  <input type="text"  class="form-control" value="{{$value}}" 
                                                          @if($template['id'] == 4)
                                                            name="school_name" pattern="^[a-zA-ZàâäèéêëîïôœùûüÿçÀÂÄÈÉÊËÎÏÔŒÙÛÜŸÇ0-9 .&]+$"
                                                          ...
                                                          if(count($arr_validations)>0)
                                                          {
                                                            ...
                                                            @if(in_array('email',$arr_validations))
                                          ...
                                          @elseif($template['get_question_category']['slug'] == 'address')
                                                <input type="text" name="address" id="location" class="form-control" value="{{$value}}"  
                                                @if($template['is_required']==1) 
                                                  data-rule-required="true" 
                                                @endif  
                                                /> 
                                          ...
                                          @elseif($template['get_question_category']['slug'] == 'latitude')
                                                 <input type="text" name="{{strslug($label)}}" id="latitude" class="form-control" value="{{$value}}"
                                                  @if($template['is_required']==1)
                                                    data-rule-required="true"
                                                  @endif 
                                                  <?php
                                          ...
                                          @elseif($template['get_question_category']['slug'] == 'longitude')
                                                 <input type="text" name="{{strslug($label)}}" id="longitude" class="form-control" value="{{$value}}"
                                                  @if($template['is_required']==1)
                                                    data-rule-required="true"
                                                  @endif  
                                                  <?php
                                                ...
                                                ?>
                                                    ...
                                                    <div class="fileupload fileupload-new" data-provides="fileupload">
                                                       <div class="fileupload-new img-thumbnail profile-img img" >
                                                          ...
                                                          @if(isset($value) && $value!=''  && file_exists($image_path.$value))
                                                 ...
                                                 @elseif($template['get_question_category']['slug'] == 'dropdown')   
                                                    <select class="form-control" name="{{strslug($label)}}"> 
                                                      ...
                                                      ?>
                                                      @foreach($arr_options as $option)
                                                        <option value="{{$option}}" @if($valuee==$option) selected @endif >{{$option}}</option>   
                                                      @endforeach
                                                    </select>  
                                                    <span class='help-block'>{{ $errors->first(strslug($label)) }}</span>
                                                 @endif  
                    ...
                    <div class="form-group">
                      <div class="col-sm-8 col-sm-offset-4 col-lg-8 col-lg-offset-3 ">
                        <a href="{{ url($school_admin_panel_slug.'/school_admin') }}" class="btn btn-primary">{{translation('back')}}</a>                           
