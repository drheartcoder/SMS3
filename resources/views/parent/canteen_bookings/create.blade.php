...
<div id="breadcrumbs">
  <ul class="breadcrumb">
    <li>
      ...
      <a href="{{ url($parent_panel_slug.'/dashboard') }}"> {{translation('dashboard')}}
      </a>
    </li>
...
<!-- BEGIN Page Title -->
 <div class="page-title new-agetitle">
    <div>
        <h1><i class="fa fa-apple"></i> {{$module_title or ''}}</h1>
    </div>
</div>
              ...
              <div class="col-sm-12 col-md-8 col-lg-8">
                  ...
                  <div class="col-md-6 col-lg-6">
                     <div class="form-group">
                          <label class="col-xs-12 col-sm-4 col-md-4 col-lg-4 control-label">{{translation('booking_date')}}</label>
                ...
                <div class="row">
                  <div class="col-sm-12 col-md-8 col-lg-8"  id="meal_div" >
                    <div class="row">
                        ...
                        <div class="dashboard-statistics " id="cart">
                          <div class="details-infor-section-block">{{translation('view_cart')}}</div>
                              <div class="" id="cart_div">
                                 ...
                                 <div class="content-list-li">
                                    <div class="img-content-prnt">
                                      @if(isset($data['get_product_details']['product_image']) && $data['get_product_details']['product_image'] != '' && file_exists($base_path.$data['get_product_details']['product_image']))
                                        ...
                                        <img src="{{$img_path.$data['get_product_details']['product_image']}}" alt="" />  
                                      @else
                                ...
                                @else
                                  <div>
                                    <h4 style="color:red;text-align: center">{{translation('your_cart_is_empty')}}</h4>
                                  </div>
                                @endif
                              </div>
                        </div>
                        <div id="checkout_div" hidden="true">
                          <div class="row">
                            <div class='col-sm-6 col-md-4 col-lg-4'></div>
                             ...
                             <div class='col-sm-6 col-md-4 col-lg-4' style="text-align: left" id="div_total"><span id="all_total">{{isset($total)?number_format($total,2):0}}</span> MAD</div>
...
function add_to_cart(elem)
  {  
      ...
      else
      {
        ...
        $.ajax({
                ...
                type:'POST',
                data:{'date':date,'item_id':item_id, 'item_price':item_price,  'daily_id':daily_id, 'item_qty' : item_qty, 'max_qty' : item_maxqty, 'stock_sold' : stock_sold, 'cust_id' : cust_id, '_token':'<?php echo csrf_token();?>'},           
  ...
  $('#datepicker').on('changeDate',function(){
        var date = $('#datepicker').val();
        $.ajax({
                    url:"{{$module_url_path.'/get_meals'}}",
                    type:'POST',
                    data:{ 'date':date,'_token':'<?php echo csrf_token();?>'},           
                      success:function(data)
                      {
                        $('#meal_div').empty();
                        $('#meal_div').append(data);
                      }
