                        ...
                        <div class="dash-white-main">
                            <div data-responsive-tabs class="verticalslide">
                                <nav>
                                    ...
                                    <ul class="content-d">
                                      @if(!empty($arr_professors))
                                        @foreach($arr_professors as $professor)
                                        <li>
                                            <a onclick="getChatDiv('{{isset($professor['user_details']['id'])?base64_encode($professor['user_details']['id']):base64_encode(0)}}')">
                                                <span class="travles-img active">
                                                    {{-- <span class="travles-green"></span> --}}
                                                    @if(isset($professor['user_details']['profile_image']) && $professor['user_details']['profile_image'] !='')
                                                      <img src="{{$profile_image_public_img_path}}/{{$professor['user_details']['profile_image']}}" alt="" />
                                                    @else
                                                      <img src="{{url('/')}}/images/default-profile.png" alt="" />
                                                    @endif
                                                </span>
                                                <span class="travles-name-blo">
                                                    <span class="travles-name-head">{{isset($professor['user_details']['first_name']) ? ucfirst($professor['user_details']['first_name']) :'' }} {{isset($professor['user_details']['last_name']) ? ucfirst($professor['user_details']['last_name']) :'' }} </span>
                                                    <span class="travles-name-sub"></span>
                                                </span>
                                                <div class="hover-info">{{getProfessorTeachingSubject($professor['user_details']['id'],Session::get('academic_year'),$level_class)}}</div>
                                            </a>
