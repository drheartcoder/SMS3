                ...
                <div class="login-wrapper">
                    <div class="main-loginpage">
                        ...
                        <div class="login-content">
                            <!-- BEGIN Login Form -->
                           <form id="form-login" action="{{ url('/') }}/{{ $parent_panel_slug }}/process_login" method="post" <?php if( Session::has('pageType')&& Session::get('pageType')=='forgetPassword'){ ?> style="display: none;" <?php } ?>>
                               ...
                               {{--  <div class="form-group">
                                    ...
                                    <select name="school_id" class="form-control" data-rule-required="true">
                                        @if(!empty($arr_schools))
                                            <option value="">Select School Name</option>
                                            @foreach($arr_schools as $key => $value)

                                            @if(isset($value['get_school_profile']['value'])&&$value['get_school_profile']['value']!='')
                                            <option value="{{$value['school_id']}}">
                                                {{ $value['get_school_profile']['value'] }}
                                ...
                                <div class="form-group">
                                    ...
                                    <span  id="err_email" style="color: red;font-size: 12px"> </span>
                                    <div class="icn-login-stn">
                           ...
                           <form id="form-forgot"  action="{{ url($parent_panel_slug.'/process_forgot_password') }}" method="post" <?php if( Session::has('pageType')&& Session::get('pageType')=='forgetPassword'){ ?> style="display: block;" <?php }else{ ?>  style="display:none" <?php } ?> >
                            {{ csrf_field() }}
                                ...
                                <hr>
                                 ...
                                 @if (Session::has('flash_notification.message') &&  Session::has('pageType')&& Session::get('pageType')=='forgetPassword')
                                    <div class="alert alert-{{ Session::get('flash_notification.level') }}">
                                        <button type="button" class="close" style="margin-top: 0px !important;padding: 0px !important;" data-dismiss="alert" aria-hidden="true">&times;</button>
                                ...
                                <div class="form-group">
                                    <div class="controls">
                                        ...
                                        <span id="err_forget_email" style="color: red;font-size: 12px;"></span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="controls btn-loginpg">
    ...
    <!--flaty scripts-->
      ...
      <script type="text/javascript">
        ...
        $('#form-login').submit(function(){
            ...
            if(email == "")
            {
               $("#err_email").text("This field is required").css('color','red');   
               return false;
            }
            else if(!pattern.test(email) )
            {          
                $("#err_email").text("Please enter valid email").css('color','red');
                return false;
            }
            else
            {
        ...
        $('#form-forgot').submit(function(){
            ...
            if(email == "")
            {
               $("#err_forget_email").text("This field is required").css('color','red');  
               return false;
            }
            else if(!pattern.test(email))
            {          
                $("#err_forget_email").text("Please enter valid email").css('color','red');
                return false;
            }
            else
            {
